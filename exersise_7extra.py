...
def generate_stats(char_class):
    ...
    binary_stat = random.choice(["Health", "Strength", "Magic", "Initiative"])
    hex_stat = random.choice(["Health", "Strength", "Magic", "Initiative"])
    while hex_stat == binary_stat:
        hex_stat = random.choice(["Health", "Strength", "Magic", "Initiative"])
    ...
    # Randomly choose a stat to print in binary, hex or decimal
    stats = {}
    for stat in ["Health", "Strength", "Magic", "Initiative"]:
        if stat == binary_stat:
            stats[stat] = bin(eval(stat.lower()))[2:]
        elif stat == hex_stat:
            stats[stat] = hex(eval(stat.lower()))[2:]
        else:
            stats[stat] = str(eval(stat.lower()))
    ...
    for char in characters:
        ...
        for stat, value in char.items():
            if stat != "Name" and stat != "Class":
                if stat == binary_stat:
                    print(f"  {stat}: 0b{value}")
                elif stat == hex_stat:
                    print(f"  {stat}: 0x{value}")
                else:
                    print(f"  {stat}: {value}")
        print()
    print("---------")
